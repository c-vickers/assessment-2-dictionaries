...
def find_unique_common_items(list1, list2):
    ...
    for item in list1:
        list1_no_dupes.add(item)

    for item in list2:
        list2_no_dupes.add(item)
    
    return list1_no_dupes & list2_no_dupes
...
def count_unique(input_string):
    ...
    text_by_word = input_string.split(" ")
    
    for word in text_by_word:
        if word in count_unique:
            count_unique[word] += 1
        count_unique.setdefault(word, 1)
...
def sort_by_word_length(words):
    ...
    for word in words:
        ...
        if length in words_by_len.keys():
            words_by_len[length].append(word)
        else:
            words_by_len[length] = [word]
    
    return words_by_len.items()
...
def get_sum_zero_pairs(input_list):
    ...
    """
###Anne - I am struggling with comparing the list to the rest of the list. And I have no idea how to use a dictionary in this.

    # zero_pairs = []

    # for i in input_list:
    #     if i + input_list[range(len(input_list)] == 0
    #             zero_pairs.append([item, n_item])
